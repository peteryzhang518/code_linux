...
int main(void) {
    // ?? GPIOC ? TIM6 ??
    (*(volatile int*)(RCC_BASE_ADDR + RCC_APB2ENR_OFFSET)) |= (1 << 2); // GPIOC

    (*(volatile int *)(RCC_BASE_ADDR + RCC_APB1ENR_OFFSET)) |= (1 << 4); // TIM6

    // ?? PC13 ???????,?????10MHz
    (*(volatile int *)(GPIOA_BASE_ADDR + GPIO_CRH_OFFSET)) &= ~(0xF << 0); // ?? PC13 ????
    (*(volatile int *)(GPIOA_BASE_ADDR + GPIO_CRH_OFFSET)) |= (0x3 << 0);   // ?? PC13 ???????, 10MHz

    // ?? TIM6 ??????
    (*(volatile int *)(TIM6_BASE_ADDR + TIM_CR1_OFFSET)) &= ~(1 << 0); // ?? TIM6
    (*(volatile int *)(TIM6_BASE_ADDR + TIM_PSC_OFFSET)) = 7199;       // ?????,72MHz / (7199 + 1) = 10kHz
    (*(volatile int *)(TIM6_BASE_ADDR + TIM_ARR_OFFSET)) = 9999;        // ?????????,10kHz / (999 + 1) = 10Hz
    (*(volatile int *)(TIM6_BASE_ADDR + TIM_DIER_OFFSET)) |= (1 << 0);  // ?? TIM6
    (*(volatile int *)(TIM6_BASE_ADDR + TIM_CR1_OFFSET)) |= (1 << 0);  // ?? TIM6
